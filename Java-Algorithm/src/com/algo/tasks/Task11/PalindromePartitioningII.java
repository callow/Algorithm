...
/**
 * 切回文问题 ： https://leetcode.com/problems/palindrome-partitioning-ii/
 * 
 */
public class PalindromePartitioningII {
	...
	public static int minCut(String s) {
		...
		dp[N] = 0;
		for (int i = N - 1; i >= 0; i--) {
			if (checkMap[i][N - 1]) { // i~N-1整体就是一个回文-> 则1个部分
				dp[i] = 1;
			} else { // i ~ n-1整体不是一个回文，若 i ~j上是回文，就用j+1的值 循环求一个最小值 得到一个格子的值
	...
	/**
	 *  建立一张正方形Map， 预处理结构，返回任何范围从L~R是否是回文
	 */
	public static boolean[][] createCheckMap(char[] str, int N) {
		boolean[][] ans = new boolean[N][N];
		for (int i = 0; i < N - 1; i++) {
			ans[i][i] = true;  // 对角线全是true 
			ans[i][i + 1] = str[i] == str[i + 1]; // 倒数第二条对角线，2个字符相等是true反之false
		}
		ans[N - 1][N - 1] = true;
		for (int i = N - 3; i >= 0; i--) {
			for (int j = i + 2; j < N; j++) {
				ans[i][j] = str[i] == str[j] && ans[i + 1][j - 1]; // 状态转移方程:每个格子 i = j且中间必须是回文
			}
		}
		return ans;
	}
