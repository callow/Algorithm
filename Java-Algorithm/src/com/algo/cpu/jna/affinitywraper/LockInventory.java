...
import com.algo.cpu.jna.cpudetails.CpuLayout;
...
class LockInventory {
    ...
    public final synchronized void set(CpuLayout cpuLayout) {
        ...
        for (int i = 0; i < cpuLayout.cpus(); i++) {
            ...
            System.out.println("cpu {} base={} reservable= {}" + i + base + reservable);
    ...
    public final synchronized AffinityLock acquireLock(boolean bind, int cpuId, AffinityStrategy... strategies) {
        ...
        try {
            if (specificCpuRequested && cpuId != 0) {
                if (cpuId > logicalCoreLocks.length) {
                	System.out.println("Unable to acquire lock on CPU {} for thread {}, as not enough CPUs" + cpuId +  Thread.currentThread());
                    return noLock();
                }
        ...
        System.out.println("No reservable CPU for {}" + Thread.currentThread());
    ...
    public final synchronized AffinityLock tryAcquireLock(boolean bind, int cpuId) {
        ...
        final AffinityLock required = logicalCoreLocks[cpuId];
        try {
            if (required.canReserve(true) && updateLockForCurrentThread(bind, required, false)) {
                return required;
            }
        } catch (ClosedByInterruptException e) {
            Thread.currentThread().interrupt();
        ...
        System.out.println("Unable to acquire lock on CPU {} for thread {}, trying to find another CPU" + cpuId +  Thread.currentThread());
    ...
    public final synchronized AffinityLock acquireCore(boolean bind, int cpuId, AffinityStrategy... strategies) {
        ...
        System.out.println("No reservable Core for {}" + Thread.currentThread());
        return acquireLock(bind, cpuId, strategies);
    }

    public final synchronized void bindWholeCore(int logicalCoreID) {
        if (logicalCoreID < 0) {
        	System.out.println("Can't bind core since it was not possible to reserve it!");
            return;
        }
        ...
        for (AffinityLock al : physicalCoreLocks.get(core)) {
            if (al.isBound() && al.assignedThread != null && al.assignedThread.isAlive()) {
                System.out.println("cpu {} already bound to {}" + al.cpuId() +  al.assignedThread);
            } else {
                al.bound = true;
    ...
    private void releaseAffinityLock(final Thread t, final AffinityLock al, final String format) {
        System.out.println(format+ al.cpuId() + t);
