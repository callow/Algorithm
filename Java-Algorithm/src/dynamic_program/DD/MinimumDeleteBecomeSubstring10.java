...
/**
 * 删除至少几个字符可以变成另一个字符串的子串: 给定两个字符串s1和s2 返回s1至少删除多少字符可以成为s2的子串
 * 
 * 
 */
public class MinimumDeleteBecomeSubstring10 {
	...
	/**
	 * dp[i][j] = s1前i个字符 能匹配 变成s2前j个字符字串的后缀串
	 * 
	 * 1. s1[i-1] != s2[j-1] 最后一个字符不等 
	 * 	  则 至少要删了它 然后看s1[0..i-2] 即 1+ dp[i-1][j]
	 * 2.  s1[i-1] = s2[j-1] 最后一个字符相等
	 * 	  则 保留， 继续看剩下的  即 dp[i-1][j-1]
	 * 
	 * 
	 */
