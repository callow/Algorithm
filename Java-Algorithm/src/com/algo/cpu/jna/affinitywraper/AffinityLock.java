...
import com.algo.cpu.jna.cpudetails.VanillaCpuLayout;
...
public class AffinityLock implements Closeable {
    ...
    static {
        ...
        try {
        	...
        	System.out.println("Unable to load /proc/cpuinfo");
        }
        PROCESSORS = processors;
        BASE_AFFINITY = Affinity.getAffinity();
        RESERVED_AFFINITY = getReservedAffinity0();
        LOCK_INVENTORY = new LockInventory(cpuLayout == null ? /*new NoCpuLayout(PROCESSORS)*/ null : cpuLayout);
    }
    ...
    static class Warnings {
        static void warmNoReservedCPUs() {
            if (RESERVED_AFFINITY.isEmpty() && PROCESSORS > 1) {
            	System.out.println("No isolated CPUs found, so assuming CPUs 1 to {} available." +  (PROCESSORS - 1));
            }
        }
    }
    ...
    public static AffinityLock acquireLock(int[] cpus) {
        ...
        System.out.println("Failed to lock any CPU in explicit list " + Arrays.toString(cpus));
