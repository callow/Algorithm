...
public class BricksFallingWhenHit {
	...
	public static int[] hitBricks(int[][] grid, int[][] hits) {
		...
		for (int i = hits.length - 1; i >= 0; i--) { // 炮弹逆序恢复
			if (grid[hits[i][0]][hits[i][1]] == 2) {
				ans[i] = unionFind.finger(hits[i][0], hits[i][1]); // 神来一指
			}
		}
		return ans;
	}
	...
	public static class UnionFind {
		...
		private int find(int row, int col) {
			int stackSize = 0;
			int index = row * M + col; // 二维数组用1维数组表示： 行号 *列数 + col
		...
		private void union(int r1, int c1, int r2, int c2) {
			if (valid(r1, c1) && valid(r2, c2)) { // 2个位置都不越界 且都要是1才开始连
				int father1 = find(r1, c1); // 找代表节点
				...
				if (father1 != father2) {
					...
					if (size1 <= size2) {
						...
						if (status1 ^ status2) { // 如果一个是天花板集合，一个不是
							cellingSet[father2] = true; // 以father2做总集合
							cellingAll += status1 ? size2 : size1; // 天花板集合的数量讲不是天花板集合的数量添加进去
						}
					} else {
		...
		public int finger(int row, int col) {
			...
			fatherMap[cur] = cur;
			sizeMap[cur] = 1;
			int pre = cellingAll; // 连之前天花板集合数量
			union(row, col, row - 1, col);
			union(row, col, row + 1, col);
			union(row, col, row, col - 1);
			union(row, col, row, col + 1);
			int now = cellingAll; // 连完后天花板集合数量
